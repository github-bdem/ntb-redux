...
class NuclearThroneAI {
  ...
  private logAIState(action: GameAction, processingTime: number): void {
    ...
    console.log(`[${runtime}s] ⚡${processingTime}ms | 🎯${action.confidence.toFixed(2)} | 🏃(${action.movement.x.toFixed(2)},${action.movement.y.toFixed(2)}) | 🎯(${action.aim.x.toFixed(0)},${action.aim.y.toFixed(0)}) | 💥${action.shooting ? '🔥' : '⭕'}`);
  }
  ...
  private async saveSession(): Promise<void> {
    ...
    const filename = `nuclear_throne_ai_session_${timestamp}.json`;
